...
<script type="text/javascript">
	...
	const remove = document.querySelector(".btnRemove").addEventListener("click", e =>{ // 상품 삭제에 대한 이벤트 처리 
	...
	document.querySelector('.productImages').addEventListener('click', e => {
		...
		const divObj = target.closest('.col-md-3');	// 하나의 이미지를 표현하는 <div>를 찾음 -> 삭제
		divObj.remove();
	});
	...
	document.querySelector(".btnModify").addEventListener("click", e =>{ 
		...
		if(imageArr.length > 0){
			...
			// beforebegin(요소 바로 앞), afterbegin(요소 내부 맨 앞), beforeend(요소 내부 맨 뒤), afteren(요소 바로 뒤)
