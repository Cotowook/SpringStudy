...
public class LogAspect {
	// 특정 객체의 메소드가 실행될 때 전달되는 파라미터를 자동으로 옮김
	// 타켓이 메소드의 실행전에 동작할 것을 명시
	// execution() : 포인트컷 표현식 중 하나로 문자열로 포인트컷 설정을 지정
	// * org.zerock.service.*.*(..) : 
	// org.zerock.service 패키지 내에 모든 클래스의 모든 메소드가 실행 전에 adivice가 동작하도록 지정
	// 리턴 타입(맨 앞)이나 매개변수(..) 상관없이 모든 메소드
	@Before("execution(* org.zerock.service.*.*(..))")
	public void loginparam(JoinPoint jp) { // JoinPoing : 현재 실행 중인 메소드 정보, 전달된 파라미터, 타겟 객체 등
		log.info("-------");
		log.info("logParam()");
		log.info("-------");
